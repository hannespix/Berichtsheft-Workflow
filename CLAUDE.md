...
**Kompaktierung schlägt fehl?** `App._compactGrund` hält den letzten Ablehnungsgrund (Zweit-Registerkarte, Sperre belegt samt Halter/Alter, fremder Snapshot, keine Datei); er steht in der Fehlermeldung, in der Nachhol-Warnung und in der Datenbank-Tools-Karte. Eine Zweit-Registerkarte (`_tabIsPrimary === false`) kann nie kompaktieren – Import und Datenbank-Tools sind dort gesperrt.

...
> **Stand:** Stufe 1 (Pakete A–E), Stufe 2 und der Großteil von Stufe 3 sind umgesetzt – siehe Git-Historie und die Abschnitte „Stufe 2 (1–4)" / „Stufe 3 (1–3)" in `tests/planung-test.mjs`, `tests/kontrolltag-test.mjs`, `tests/status-test.mjs`, `tests/workflow-test.mjs`, `tests/berichte-test.mjs`. **Stufe 3 umgesetzt:** 1 Startseite/Schnellstart, 2 Jahreskalender (Kampagnenfenster, Blockwochen, Ferien BW, „Diese Woche"), 3 Hersendung (Eingangsliste „Heft da", Erinnerung an Betriebe), 4 Ausschluss bereits kontrollierter Azubis + Nachholtermine, 5 Mahnstufen/Versandprotokoll + Sammel-Erinnerung je Betrieb + Versandhistorie im Aktenvermerk, 6 Betriebs-/Schul-Ampel, 8 teilweise (Blockplan „Vorjahr kopieren"/„aus Text", LFK-Regeln als Tabelle; kein Datei-Import je Schule), 9 Kontexthilfe (F1, ?-Link, „Was ist neu"), 10 teilweise (Kontrolltag-Cockpit mit Startzeit/Ø/Prognose, Begründung für den Prüfungsausschuss; keine Tablet-Tap-Leiste, kein Foto je Durchsicht), 11 Rollenprofile. 12 Offline am Kontrollort ist umgesetzt (Feldmodus, lokaler Stand, Zusammenführung mit Konfliktliste, `tests/feldmodus-test.mjs`, `tests/offline-test.mjs`); alte Termine werden per 90-Tage-Regel ausgeblendet. **Offen:** 7 eine Azubi-Detailseite statt drei Modale, Blockplan-Import aus CSV/Excel-Datei. Hinweis zu Stufe 2: `App.confirm()/App.prompt()` stehen als Promise-Dialoge bereit (genutzt für Nachholtermin und Prüfungsausschuss), die übrigen `window.confirm`-Aufrufe sind noch nicht umgestellt.

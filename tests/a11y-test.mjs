...
console.log('\n══ Feinschliff Durchsicht: eine haftende Zeile ══');
{
  ...
  check(/--gutter: 32px;/.test(CSS) && /\.main-content \{\n  flex: 1;\n  overflow-y: auto;\n  padding: 0 var\(--gutter\) var\(--gutter-unten\);/.test(CSS) && /padding: 0 var\(--gutter\);\n  margin: 0 calc\(-1 \* var\(--gutter\)\) 0;/.test(CSS) && /margin: 12px calc\(-1 \* var\(--gutter\)\) calc\(-1 \* var\(--gutter-unten\)\); padding: 0 var\(--gutter\) 8px;/.test(CSS), 'Ein Randmaß für Inhalt, Azubi-Kopf und Ergebnisleiste');

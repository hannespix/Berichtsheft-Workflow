...
console.log('\n══ Netzabriss: erkennen, pausieren, Probe, weiter ══');
{
  ...
  const echterDir = E.dirHandle, echterDbDir = E.dbDirHandle, echteDb = E.dbFileHandle;
  // Bei echtem Netzabriss sterben ALLE Zugriffspunkte – auch der auf die Datenbankdatei
  E.dirHandle = tot; E.bhkDirHandle = null; E.dbDirHandle = null;
  E.dbFileHandle = { kind: 'file', name: echteDb.name, getFile: async () => nf(), createWritable: async () => nf() };
  ...
  E.dirHandle = echterDir; E.dbDirHandle = echterDbDir;
  check((await E._netzProbe(false)) === true && !E._netzWeg && E._verbFehler === 0 && E.dbFileHandle !== echteDb && E.dbFileHandle.name === echteDb.name, 'Laufwerk wieder da: Probe hebt die Pause auf und holt den Datei-Zugriffspunkt frisch');

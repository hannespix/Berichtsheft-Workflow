...
const db = new SQL.Database(makeSeed(SQL, `
  ...
  UPDATE schueler SET klasse_id=1, ausbildungsstaette='Gärtnerei Test', ibykus_id='I-1' WHERE id=1;
...
console.log('\n══ Seite rendern: Abschnitte statt Dialoge ══');
{
  ...
  for (const k of ['stammdaten', 'phasen', 'akte', 'kontrollen', 'wiedervorlagen']) check(new RegExp(`id="azAbschnitt_${k}"`).test(main) && new RegExp(`id="azTeil_${k}"`).test(main), `Abschnitt ${k}`);
...
console.log('\n══ Aktionen auf der Seite ══');
{
  ...
  check(App.currentView === 'azubi', 'Öffnen mit Sprungziel');
  AzubiSeite.zurueck();

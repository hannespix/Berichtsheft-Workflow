...
console.log('\n══ Oberfläche (Quellen) ══');
{
  ...
  check(/App\.run\('DELETE FROM bhk_papierkorb WHERE id=\?', \[pkId\]\)/.test(V) && /if \(App\.scalar\("SELECT COUNT\(\*\) FROM wiedervorlagen WHERE status='offen' AND frist_datum < \?"/.test(V), 'Papierkorb-Rest ohne Dateilöschung; Überfällig-Update nur bei Treffern');
  ...
  check(/Workflows\._individualData\.betriebe\[\$\{idx\}\]\.key/.test(WF) && /Workflows\._nachholung\.betroffen\[\$\{idx\}\]\.key/.test(WF) && /status IN \('offen','ueberfaellig'\) ORDER BY w\.frist_datum LIMIT 1/.test(WF) && /if \(yz > 270\)/.test(WF), 'Serienbrief: Schlüssel per Index statt Zeichenkette, Frist auch bei überfällig, Seitenumbruch');

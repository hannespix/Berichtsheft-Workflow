...
const App = {
  ...
  async enableWriteAccess() {
    ...
    try {
      ...
      // Get/create _bhk dirs (und Datenbanken/ – die Datei kann dort liegen)
      await this.ensureAppDirs();
      this.dbFileHandle = await this._dbDateiHandle(name);
  ...
  // Die Datenbankdatei kann im Hauptordner ODER in Datenbanken/ liegen. Erst
  // den bekannten Zugriffspunkt selbst prüfen, dann in beiden Ordnern suchen.
  // Vorher suchte die Netzprobe nur im Hauptordner: Eine neue Datenbank in
  // Datenbanken/ galt damit als „nicht erreichbar“ – Speichern pausiert,
  // Import blockiert, obwohl das Laufwerk da war.
  async _dbDateiHandle(name) {
    name = name || (this.dbFileHandle && this.dbFileHandle.name) || this.autoLoadedDbName;
    if (!name) throw new Error('keine Datenbankdatei bekannt');
    if (this.dbFileHandle && this.dbFileHandle.name === name) {
  ...
  async _netzProbe(manuell) {
    ...
    const t0 = Date.now();
    try {
      const h = await this._dbDateiHandle();
      this.dbFileHandle = h;
    ...
    try {
      ...
      if (e.name === 'InvalidStateError' && this.dirHandle && this._writeRetryCount <= 3) {
        try {
          ...
          this.dbFileHandle = await this._dbDateiHandle(oldName);
  ...
  async tryReconnect() {
    ...
    try {
      ...
      if (this.dirHandle && this.dbFileHandle?.name) {
        try {
          this.dbFileHandle = await this._dbDateiHandle(this.dbFileHandle.name);
          const file = await this.dbFileHandle.getFile();
          this._reconnectAttempts = 0;
  ...
  run(sql, params = []) {
    // Bulk-Modus (Import, Bereinigung): keine Änderungsverfolgung je Anweisung,
    // aber DIESELBE Vorbereitung wie eine normale Op – globale Kennung in
    // INSERTs, eingefrorene Zeit, Tombstones. Vorher wurde das SQL roh
    // ausgeführt; die gemerkten _bulkOps trugen keine Kennungen und taugten
    // nur zur Wiederholung auf demselben Rechner. Jetzt sind sie vollwertige
    // Ops: bulkAlsOps() übergibt sie nach dem Import dem Protokoll, und alle
    // Kollegen bekommen dieselben Kennungen (kein Snapshot, keine Sperre).
    if (this._bulkImport) {
      const prep = this._prepareOp(sql, params);
      if (!this._bulkOps) this._bulkOps = [];
      const merke = (q, p) => { if (this._bulkOps.length < 200000) this._bulkOps.push({ sql: q, params: [...(p || [])] }); };
      prep.pre.forEach(x => { try { this.db.run(x.sql, x.params); merke(x.sql, x.params); } catch(e) {} });
      this.db.run(prep.sql, prep.params);
      merke(prep.opSql, prep.opParams);
      prep.post.forEach(x => { try { this.db.run(x.sql, x.params); merke(x.sql, x.params); } catch(e) {} });
      return;
    }
    const prep = this._prepareOp(sql, params);
  ...
  _bulkImport: false,
  // Im Bulk-Modus ausgeführte Anweisungen (ab Index `ab`) als normale Ops in
  // den Puffer übernehmen: Stempel, Kennung, betroffener Azubi, Absturzpuffer,
  // Anhängen im Hintergrund in Häppchen (APPEND_MAX_BYTES). Der Import
  // wartet damit nicht mehr auf eine Kompaktierung mit Sperre und Zeitlimit
  // (über eine langsame Leitung 300 s und mehr, und bis dahin standen die
  // Daten nur im Speicher). Liefert die Zahl der übernommenen Ops.
  bulkAlsOps(ab = 0, quelle = 'Import') {
    const ops = (this._bulkOps || []).slice(ab);
    if (this._bulkOps) this._bulkOps.length = Math.min(this._bulkOps.length, ab);
    if (!ops.length) return 0;
    const cid = this._getClientId();
    for (const o of ops) {
      const rec = { uid: this._newUid(), ts: this._stampTs(), seq: this._nextSeq(), sql: o.sql, params: o.params };
      const sid = this._azubiAusOp(rec.sql, rec.params);
      if (sid != null) { rec.sid = sid; this._ungesichertAzubis.add(sid); }
      this._dirtyOps.push(rec);
      this._notiereStamp(rec.sql, rec.params, rec.ts, cid, rec.seq);
    }
    try { if (typeof GlobalSearch !== 'undefined') GlobalSearch._hayCache = null; } catch(e) {}
    this.markDirty();
    BhkSpur.notiere('anhaengen', 'Bulk-Ops übernommen', { ok: true, info: `${quelle}: ${ops.length} Änderung(en) in den Puffer` });
    // Sofort in den Absturzpuffer und dann anhängen – ohne zu warten: die
    // Oberfläche bleibt frei, der Speicherstatus zeigt „n wartend“.
    this.sofortSpeichern(quelle).catch(() => {});
    return ops.length;
  },
  ...
  async _saveV3() {
    ...
    // Höchstens APPEND_MAX_BYTES je Anhängen (ein Import bringt Tausende
    // Ops): ein einziger Megabyte-Schreibvorgang liefe über eine langsame
    // Leitung ins Zeitlimit; Häppchen kommen nacheinander durch, und nach
    // jedem rotiert das Protokoll (kein Kopieren großer Dateien). Der Rest
    // bleibt im Puffer und folgt mit dem nächsten Auto-Save.
    ...
    const claimed = this._dirtyOps.splice(0, anzahl);
    this._wartetSeit = 0;
    this._opsInFlight = claimed; // für _persistDirtyOps: Crash-Puffer behält sie
    let writable = null;
    try {
      const dir = this._syncDirV3();
      const bytes = new TextEncoder().encode(zeilen.join(''));
  ...
  async _pruefeFremdenSnapshot() {
    ...
    try {
      ...
      // Vor der ersten Kompaktierung gibt es kein snapmeta – das ist kein
      // Fehler und erst recht kein Netzabriss (kostete eine neue Datenbank
      // sonst den Import: zwei Fehlzählungen → „nicht erreichbar“)
      if (BhkSpur.fehlerArt(e) === 'nicht-gefunden') return;
  ...
  getSchuelerAJs(schuelerId) {
    ...
    if (typeof Phasen !== 'undefined') {
      try {
        ...
        if (phasen.length) {
          ...
          // Das berechnete Phasen-Ende ist der Tag NACH dem Vertrag (Beginn + n
          // Monate, z.B. 1.9.); das Vertragsende ist der Tag davor (31.8.) –
          // sonst zählte ein Vertrag bis 31.8. ein Schuljahr zu viel
          if (berechnetesEnde) { d2 = new Date(berechnetesEnde); d2.setDate(d2.getDate() - 1); }
        }
      } catch(e) {}
    }
    if (!d2) return [1, 2, 3];
    ...
    // Erstes Ausbildungsjahr aus der Vertragsdauer: 36 Monate → 1, 24 → 2
    // (Verkürzer steigen im 2. Lehrjahr ein), 12 → 3; Verlängerer ab 1.
    const jahreVertrag = Math.max(1, Math.ceil(monate / 12));
    const erstesAJ = Math.max(1, Math.min(3, 4 - jahreVertrag));
    // ANZAHL der Raster = überspannte Schuljahre (Sep–Aug, August zählt zum
    // kommenden Schuljahr, s. _schuljahrBeginn): Die Raster sind Schuljahre,
    // nicht Vertragsjahre. Aus der Dauer gezählt fehlte einem Vertrag ab 1.3.
    // (oder 1.7.) das letzte Schuljahr – die Wochen des letzten Vertragsjahres
    // hatten kein Raster. Für Verträge ab 1.8./1.9. ändert sich nichts.
    const anzahlSJ = Math.max(1, Math.min(5, this._schuljahrAm(d2) - this._schuljahrBeginn(d1) + 1));
    return Array.from({ length: anzahlSJ }, (_, i) => erstesAJ + i);
  },
  // Schuljahr-Zuordnung der Raster (Sep–Aug): Beginnt ein Vertrag im August,
  // zählt er zum KOMMENDEN Schuljahr – die Augustwochen KW 31–35 liegen am Ende
  // des Rasters (getAJKWBounds: „ganzes Raster aktiv“). Ein beliebiges Datum
  // (Stichtag, heute, Vertragsende) gehört im August noch zum ablaufenden Schuljahr.
  _schuljahrBeginn(d) { return d.getMonth() >= 7 ? d.getFullYear() : d.getFullYear() - 1; },
  _schuljahrAm(d) { return d.getMonth() >= 8 ? d.getFullYear() : d.getFullYear() - 1; },
  // Raster (Ausbildungsjahr) zu einem Datum: Schuljahr des Datums relativ zum
  // ersten Raster. Vorher „Monate seit Beginn / 12 + 1“ – für Verkürzer mit
  // Raster [2,3] ergab das im zweiten Jahr weiter AJ 2 (Stichtag „geprüft bis
  // KW“ landete im falschen Raster), und für Verträge ab 1.8. passte es nicht
  // zu den Schuljahres-Beschriftungen.
  _ajZumDatum(beginn, ref, ajs) {
  ...
  getCurrentAJ(beginn, schuelerId) {
    ...
    // Kalenderbasiert wie das Raster selbst (Schuljahr Sep–Aug); Teilzeit und
    // Unterbrechungen verschieben nur das Vertragsende (getSchuelerAJs), nicht
    // die Kalenderwoche, in der der Prüfer gerade steht
    return this._ajZumDatum(beginn, new Date(), ajs);
  },
  ...
  getAJAtDate(beginn, datum, schuelerId) {
    ...
    if (!ref) return null;
    const ajs = schuelerId ? (this.getSchuelerAJs(schuelerId) || [1, 2, 3]) : [1, 2, 3];
    return this._ajZumDatum(beginn, ref, ajs);
  },
  ...
  getAJKWBounds(schuelerId) {
    ...
    // Schuljahr des ersten Rasters (August zählt zum kommenden Schuljahr – vorher
    // bekam ein Vertrag ab 1.8. das Raster des VORjahres mit falschen Daten in den
    // Tooltips, und sein letztes Vertragsjahr hatte kein Raster)
    const firstSY = d1 ? this._schuljahrBeginn(d1) : null;

const SchuelerView = {
  ...
  getFilteredData() {
    ...
    if (this.sortCol === 'lehrjahr') {
      ...
      rows.sort((a, b) => ((a.eff_lehrjahr || 0) - (b.eff_lehrjahr || 0)) * dir || (a.nachname || '').localeCompare(b.nachname || ''));
    }
    return rows;
  },
      ...
      <div class="card">
        <div class="card-header">
          ...
          <div class="btn-group">
            ...
            ${totalAll ? `<button class="btn btn-sm" style="background:transparent;color:var(--clr-red);border:1px solid var(--clr-red)" onclick="ImportHandler.deleteAllJahrgang()">Alle löschen</button>` : ''}
          </div>
        </div>
        ...
        <table class="data-table table-sticky-name"><thead><tr>
          ...
          ${pageData.map(s => {
            ...
            <td class="btn-group">
              ...
              ${typeof Phasen!=='undefined'?`<button class="btn-icon btn-sm" title="Ausbildungsverlauf (Phasen: Teilzeit, Unterbrechungen, Betriebswechsel)" onclick="Phasen.editor(${s.id})">${svgIcon('dashboard')}</button>`:''}
              <button class="btn-icon btn-sm" title="Akte: Bemerkungen" onclick="SchuelerAkte.open(${s.id})">${svgIcon('akte')}</button>

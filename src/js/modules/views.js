    ...
    mc.innerHTML = `<div class="fade-in">
      ...
      <div style="margin-bottom:20px">
        ...
        <div class="grid-3" style="margin-bottom:16px">
          ...
          <!-- Mängel-Verteilung -->
          <div class="card">
            <div class="card-header">⚠︎ Häufigste Mängel-Codes</div>
...
`,
      ...
      <div class="card" style="margin-top:16px">
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:6px 10px;background:var(--clr-warm);border-radius:var(--radius);font-size:13px;flex-wrap:wrap">
          <label for="setVerbindung">Diese Leitung ist</label>
          <select id="setVerbindung" class="form-control" style="width:auto" onchange="App.setVerbindung(this.value)">
            <option value="auto" ${App.verbindungsStufe === 'auto' ? 'selected' : ''}>Automatisch (Büro-LAN, misst selbst)</option>
            <option value="langsam" ${App.verbindungsStufe === 'langsam' ? 'selected' : ''}>Langsame Leitung (VPN, Homeoffice)</option>
            <option value="getaktet" ${App.verbindungsStufe === 'getaktet' ? 'selected' : ''}>Getaktete Verbindung (Mobilfunk, SIM)</option>
          </select>
          <span style="color:var(--clr-text-light)">${App.verbindungsStufe === 'getaktet' ? 'Abgleich und Speichern alle 60 s, keine Sicherung und keine Kompaktierung von diesem Rechner, Positionsanzeige je Minute' : App.verbindungsStufe === 'langsam' ? 'Abgleich alle 30 s, Speichern gebündelt (10 s), keine Kompaktierung von diesem Rechner' : 'Takt aus der gemessenen Netzqualität (3 s / 10 s / 30 s)'}</span>
        </div>
        ...
        <div style="font-size:12px;color:var(--clr-text-light);margin-top:6px;line-height:1.6">
          ...
          Der Abgleich liest je Takt nur den <strong>Änderungszeiger</strong> (eine winzige Datei) und holt die Protokolle der Kollegen nur, wenn sich dort etwas getan hat – ein voller Rundgang läuft als Sicherheitsnetz alle ${Math.round(App.rundgangIntervallMs() / 60000)} min. Über Mobilfunk/VPN wird jede Dateioperation langsam; die Stufen „Langsame Leitung“ und „Getaktete Verbindung“ nehmen den Takt heraus. Für längere Termine ohne Netz: <strong>Offline-Modus</strong> (Schaltfläche in der Kopfzeile).
        </div>
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px;align-items:center">
          ...
          <button class="btn btn-secondary" onclick="Konsole.protokolleDialog()" title="Welche Rechner schreiben Protokolle für diese Datenbank, wie groß und wie alt sind sie, was ist hier noch ungelesen – zum Vergleich mit dem anderen Rechner">▤ Protokolle im Ordner</button>          <span style="font-size:12px;color:var(--clr-text-light)">Für die Fehlersuche je Zugangsweg (Büro, VPN, Mobilfunk): Test dort ausführen und Ergebnisse vergleichen. Mehr in der Browser-Konsole (F12): <code>bhk.hilfe()</code>, je Azubi <code>bhk.pruefen(id)</code>. Programmstand: <code>${esc(App.BUILD)}</code>${App._uhrVersatzMs ? ` · Uhrversatz zum Dateiserver ${Math.round(App._uhrVersatzMs / 1000)} s` : ''}</span>
        </div>
        <div style="margin-top:8px;padding:8px 10px;border-left:4px solid ${App.istLeitDb() === false ? 'var(--clr-amber)' : 'var(--clr-green)'};background:${App.istLeitDb() === false ? 'var(--clr-amber-light, #fff7ed)' : 'var(--clr-green-light)'};font-size:13px;line-height:1.6">
          <strong>Gemeinsame Datenbank dieses Ordners:</strong> ${App._leitDb ? `„${esc(App._leitDb.name)}“${App._leitDb.gesetztVon ? ` (festgelegt von ${esc(App._leitDb.gesetztVon)}${App._leitDb.am ? ' am ' + esc(new Date(App._leitDb.am).toLocaleDateString('de-DE')) : ''})` : ''}` : 'noch nicht festgelegt'}.
          ${App.istLeitDb() === false ? `<strong style="color:var(--clr-amber)">Hier ist aber „${esc(App.autoLoadedDbName || '?')}“ geöffnet – Kollegen sehen Änderungen hier nicht.</strong>` : App.istLeitDb() === true ? 'Diese Datenbank ist hier geöffnet – alle Kollegen öffnen sie beim Start automatisch.' : 'Jeder Rechner öffnet die gemeinsame Datei beim Start automatisch, sobald sie festgelegt ist.'}
          <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">
            ${App.istLeitDb() === false ? `<button class="btn btn-primary btn-sm" onclick="App.zurLeitDb()" title="Änderungen schreiben und mit der gemeinsamen Datenbank neu starten">→ Zur gemeinsamen Datenbank wechseln</button>` : ''}
            ${App.istLeitDb() !== true && App.autoLoadedDbName ? `<button class="btn btn-secondary btn-sm" onclick="App.dieseAlsLeitDb()" title="Die hier geöffnete Datei wird für alle Kollegen die gemeinsame">Diese Datenbank als gemeinsame festlegen</button>` : ''}
          </div>
        </div>
        <div style="font-size:13px;line-height:1.6;margin-top:8px"><strong>Arbeitsordner:</strong> „${esc(App.dirHandle ? (App.dirHandle.name || '?') : '–')}“ · Datenbank „${esc(App.autoLoadedDbName || '–')}“${App._ordnerKennung ? ` · Ordner-Kennung <code>${esc(App._ordnerKennung.id)}</code>` : ''}. Alle Kollegen müssen denselben Ordner und dieselbe Datei öffnen – jeder andere Ordner ist eine eigene Welt.</div>
        ${App._ordnerGewechselt ? `<div style="margin-top:8px;padding:8px 10px;border-left:4px solid var(--clr-amber);background:var(--clr-amber-light, #fff7ed);font-size:13px;line-height:1.6"><strong>⚠ Anderer Arbeitsordner als zuletzt:</strong> jetzt „${esc(App._ordnerGewechselt.nach)}“, zuletzt „${esc(App._ordnerGewechselt.von)}“. Wenn das nicht beabsichtigt ist: „DB wechseln“ → Verbindung trennen und den gemeinsamen Ordner wählen.</div>` : ''}
        ${(App._unterordnerMitDb || []).length ? `<div style="margin-top:8px;padding:8px 10px;border-left:4px solid var(--clr-amber);background:var(--clr-amber-light, #fff7ed);font-size:13px;line-height:1.6"><strong>⚠ Unterordner mit eigener Datenbank:</strong> ${App._unterordnerMitDb.map(f => `„${esc(f.ordner)}“ (${[f.datenbanken ? 'Datenbanken/' : '', f.bhk ? '_bhk/' : '', f.sqlite ? f.sqlite + ' × .sqlite' : ''].filter(Boolean).join(', ')})`).join(', ')}. Wählt ein Kollege beim Start diesen Ordner, arbeitet er getrennt von allen anderen. Alte Kopien bitte aus dem Arbeitsordner heraus verschieben.</div>` : ''}
        ${App._bhkFehlt ? `<div style="margin-top:8px;padding:8px 10px;border-left:4px solid var(--clr-red, #b00020);background:#fdecea;font-size:13px;line-height:1.6"><strong>⚠ Unterordner „_bhk“ nicht zugänglich</strong> (${esc(App._bhkFehlt)}). Dieser Rechner schreibt seine Protokolle in den Hauptordner – die Kollegen lesen dort nicht. Ordnerrechte prüfen, Verbindung trennen und neu verbinden.</div>` : ''}
        ${(App._protokolleAusserhalb || []).length ? `<div style="margin-top:8px;padding:8px 10px;border-left:4px solid var(--clr-amber);background:var(--clr-amber-light, #fff7ed);font-size:13px;line-height:1.6"><strong>⚠ Protokolle außerhalb von „_bhk“:</strong> Im Hauptordner liegen ${App._protokolleAusserhalb.length} aktuelle Protokoll-Datei(en) (zuletzt ${esc(new Date(App._protokolleAusserhalb[0].juengste).toLocaleString('de-DE'))}). Ein anderer Rechner konnte „_bhk“ offenbar nicht öffnen und schreibt daneben – seine Änderungen kommen hier nicht an. Dort Ordnerrechte prüfen und neu verbinden; danach die Dateien nach „_bhk“ verschieben.</div>` : ''}
        ${(App._fremdeDatenbanken || []).length ? `<div style="margin-top:8px;padding:8px 10px;border-left:4px solid var(--clr-amber);background:var(--clr-amber-light, #fff7ed);font-size:13px;line-height:1.6">
          <strong>⚠ Andere Datenbank im selben Ordner:</strong> Hier ist „${esc(App.autoLoadedDbName || '?')}“ geöffnet, im Ordner werden aber auch Protokolle für ${App._fremdeDatenbanken.map(e => `„${esc(e.slug)}“ (${e.dateien} Protokolle, zuletzt ${esc(new Date(e.juengste).toLocaleString('de-DE'))})`).join(', ')} geschrieben. Dort arbeitet vermutlich ein Kollege – und keiner sieht die Änderungen des anderen. Alle müssen dieselbe Datei öffnen (Name rechts in der Kopfzeile; beim Start die Datenbank auswählen).
        </div>` : ''}
        <div style="font-size:12px;color:var(--clr-text-light);margin-top:8px;line-height:1.6">
          <strong>Zwei Rechner zeigen für einen Azubi verschiedene Stände?</strong> Erst prüfen, ob beide dieselbe Datenbankdatei geöffnet haben (Name rechts in der Kopfzeile; <code>bhk.pruefen(id)</code> nennt Datenbank, Ordner und Protokolle anderer Datenbanken). Dann auf dem Rechner, dem etwas fehlt, „⟳ Vollabgleich“. Hilft das nicht, hat dieser Rechner die Änderungen des Kollegen <em>verworfen</em>, weil er für dieselben Felder eigene, neuere Werte hatte (Last-Write-Wins; <code>bhk.pruefen(id)</code> zeigt „verworfen“).
...
`,
      ...
      <div style="display:flex;gap:16px;align-items:flex-start">
        ...
        <div style="flex:1;min-width:0;font-size:13px;line-height:1.7">
          ...
          <div id="help_warning" class="card" style="margin-bottom:12px;border-left:4px solid var(--clr-red)">
            <div class="card-header" style="font-size:15px;color:var(--clr-red)">⚠︎ Wichtige Hinweise für neue Mitarbeiter</div>
          ...
          <div class="help-abschnitt">
            ...
            <div style="margin-top:4px;padding:10px;background:var(--clr-sand-light);border-radius:var(--radius);font-size:12px;line-height:1.8">
              <p><span style="color:var(--clr-red)">◆</span> <strong>Priorität 1 – Pflicht (~15 Min.):</strong> Offene Wiedervorlagen übernehmen – andernfalls gehen laufende Fristen verloren. Nur die aktuell offenen Wiedervorlagen eintragen (Auszubildender, Frist, Art der Wiedervorlage).</p>
              <p><span style="color:var(--clr-amber)">◐</span> <strong>Priorität 2 – Empfohlen (~30 Min.):</strong> Letztes Durchsichtsergebnis je Auszubildendem erfassen. Damit arbeitet das Ampelsystem korrekt und das Dashboard zeigt zutreffende Kennzahlen.</p>
              <p><span style="color:var(--clr-green)">●</span> <strong>Priorität 3 – Optional (~60 Min.):</strong> KW-Mängelcodes der letzten Durchsicht übernehmen, sofern der Mängelverlauf bei der nächsten Kontrolle sichtbar sein soll.</p>
              <p><span style="color:var(--clr-sage-light)">○</span> <strong>Weglassen:</strong> Ältere Durchsichten und Detail-KWs. Der Aufwand übersteigt den Nutzen – diese Daten sind auf den archivierten Papierbögen vorhanden.</p>
            </div>
          ...
          <div id="help_5" class="card" style="margin-bottom:12px;border-left:4px solid var(--clr-forest)">
            ...
            <p><strong>Höhe der Ergebnisleiste:</strong> Das Bemerkungsfeld zeigt immer den ganzen Text, bis etwa sechs Zeilen. Braucht es mehr Platz, den <strong>Griff an der Oberkante der Leiste</strong> nach oben ziehen (oder den Griff mit Tab anwählen und ↑/↓ drücken) – die gewählte Höhe bleibt für Sie bei jedem Azubi erhalten. Doppelklick auf den Griff (oder Esc darauf) setzt zurück auf automatisch. Höchstens die halbe Fensterhöhe, damit das Raster sichtbar bleibt; bei kleinen Fenstern scrollt die Leiste ohnehin mit und hat keinen Griff.</p>
          ...
          <div id="help_6" class="card" style="margin-bottom:12px">
            ...
            <div class="help-abschnitt">
              ...
              <p>• <strong>Optionen</strong> über der Liste, je Person gemerkt: <em>Ansprechpartner namentlich ansprechen</em> (Standard aus – „Sehr geehrte Damen und Herren“ reicht meist), <em>Signatur</em> (Standard: die angemeldete Person; jeder andere Prüfer wählbar; oder „keine“ – dann endet der Text vor der Grußformel und Outlook hängt beim Senden die eigene Signatur an) und <em>Durchsichtsbögen anhängen</em>. Die Vorschau in der Liste zeigt sofort den geänderten Text.</p>
              <p>• <strong>⬇ Alle als Outlook-Entwürfe (ZIP)</strong> ist der Hauptweg, ohne Längengrenze. Je Betrieb entsteht eine .eml-Datei mit Empfänger, Betreff, Text und – nach der Kontrolle – den Durchsichtsbögen der genannten Azubis als PDF-Anhang; alles zusammen in einer ZIP-Datei (mit LIESMICH). ZIP entpacken, .eml doppelklicken: das klassische Outlook öffnet sie als neuen Entwurf mit „Senden“ (das neue Outlook zeigt solche Dateien nur zum Lesen – dann den Weg „Nächste öffnen“ nutzen).</p>
              <p>• <strong>✉︎ Nächste öffnen</strong> ist der zweite Weg über den E-Mail-Link: je Klick startet Outlook mit Empfänger, Betreff und Text, die Zeile wird grün („geöffnet 10:32“). „Öffnen“ in der Zeile geht auch einzeln; „Vorschau“ zeigt den Text, „▤ Text“ kopiert Empfänger, Betreff und Text. Ein E-Mail-Link fasst nur etwa 2000 Zeichen: passt der Text nicht, sagt es die Zeile vorher; beim Öffnen startet Outlook mit Empfänger und Betreff, der Text liegt in der Zwischenablage und wird mit <strong>Strg+V</strong> eingefügt.</p>
              <p>• <strong>Versandnachweis:</strong> Das erste Öffnen einer Mängelmitteilung oder Nachhol-Aufforderung (oder das Erzeugen des Entwurfs) wird an den Wiedervorlagen der genannten Azubis vermerkt (Datum, per E-Mail, Mahnstufe 1) – die Liste zeigt dann kein „ohne Versand“ mehr. Sind alle Betriebe mit E-Mail erledigt, gilt der Termin als nachbereitet.</p>
            </div>
          </div>
          ...
          <div id="help_10" class="card" style="margin-bottom:12px;border-left:4px solid var(--clr-amber)">
            ...
            <p>• <strong>Datenbank-Tools</strong> – Bestandsübersicht (Dateigröße, Zeilen je Tabelle, Jahrgänge), <em>Verdichten</em> (Wochendaten lange inaktiver Azubis entfernen, Ergebnisse und Statistik bleiben), <em>Jahrgänge mit Archiv löschen</em> (vorher SQLite + Excel nach <code>_bhk/archiv/</code>, gegengelesen, einzelne Azubis später zurückholbar), <em>Aufräumen</em> (verwaiste Zeilen, alte Logs, Blockpläne, Sync-Stempel) und <em>Datei neu aufbauen</em> (VACUUM – erst das macht die Datei kleiner). Jede Aktion: Vorschau mit Zahlen, automatisches Backup, neuer Snapshot für alle Nutzer. Läuft gerade eine Kompaktierung (z.B. nach dem Start), wartet das Werkzeug bis zu 3 Minuten, bevor es etwas ändert; kann der Snapshot danach nicht sofort geschrieben werden, bleibt der Stand im Speicher und wird automatisch nachgeholt (Fenster offen lassen); der Grund steht im Klartext in der Karte (auch „Safe Browsing“ als Schreibhindernis), „Sperre prüfen“ zeigt, wer die Kompaktierungs-Sperre hält, und gibt sie notfalls frei; eine eigene Sperre, deren Freigabe an einem Schreibfehler scheiterte, wird automatisch wieder übernommen und später freigegeben. Nicht im Offline-Modus, nicht bei Netzabriss und nicht in einer <strong>zweiten Registerkarte</strong> derselben Datenbank – die kann grundsätzlich keinen Snapshot schreiben (auch Import ist dort gesperrt)</p>
          ...
          <div class="help-abschnitt">
            ...
            <p>• <strong>Eine gemeinsame Datenbank je Ordner:</strong> In <code>_bhk/datenbank.json</code> steht, welche Datei unter <code>Datenbanken/</code> die gemeinsame ist. Jeder Rechner öffnet sie beim Start automatisch – auch wenn der Browser sich an eine andere erinnert. Liegen mehrere Dateien im Ordner und ist noch keine festgelegt, wählt der erste Rechner einmal; die Wahl gilt für alle. Wer eine andere Datei öffnet, wird gefragt und sieht in der Kopfzeile „⚠ nicht die gemeinsame Datenbank“ (Wechsel unter <em>Wartung → Verbindung</em>)</p>
  ...
  exportHilfePDF() {
    ...
    const clean = (s) => (s||'').replace(/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}\u{1F000}-\u{1F2FF}←-⇿\u{2200}-\u{23FF}\u{25A0}-\u{25FF}\u{2B00}-\u{2BFF}\u{1F100}-\u{1F1FF}\u{FE00}-\u{FE0F}\u{20E3}]/gu, '').replace(/[​-‏﻿]/g, '').replace(/\s+/g, ' ').trim();
    ...
    doc.text('Inhaltsverzeichnis', lm, y); y += 10;
    doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(50, 50, 50);

    const chapters = cards.map((c, i) => {
      const hdr = c.querySelector('.card-header');
      return hdr ? hdr.textContent.trim() : 'Abschnitt ' + (i + 1);
    });
    // TOC-Einträge: Position + Klickfläche merken, Seitenzahl wird später nachgetragen
    const tocEntries = [];
    ...
    const pageRefs = []; // Index → Seitenzahl im PDF
    ...
    cards.forEach((card, idx) => {
      addPage();
      pageRefs[idx] = doc.getCurrentPageInfo().pageNumber;
      ...
      const headerEl = tempDiv.querySelector('.card-header');
      ...
      blocks.forEach(block => {
        ...
        if (block.querySelector('strong') && text.length < 80) {
          doc.setFont('helvetica', 'bold');
          doc.text(lines, lm, y); y += blockH + 2;
          doc.setFont('helvetica', 'normal');
        } else if (text.startsWith('•') || text.startsWith('-')) {
          doc.text(lines, lm + 4, y); y += blockH + 1;
        } else {
          doc.text(lines, lm, y); y += blockH + 2;
        }
      });
      ...
      tables.forEach(table => {
        ...
        if (rows.length) {
          ...
          try {
            doc.autoTable({
              startY: y, margin: { left: lm, right: 20 },
              head: [rows[0]], body: rows.slice(1),
              styles: { fontSize: 8, cellPadding: 2, lineWidth: 0.2 },
              headStyles: { fillColor: [240, 238, 230], textColor: [45, 80, 22], fontStyle: 'bold' },
              theme: 'grid',
            });
    ...
    // TOC-Hyperlinks nachträglich setzen + Seitenzahlen ergänzen
    tocEntries.forEach((entry, i) => {
      const targetPage = pageRefs[i];
      if (!targetPage) return;
      doc.setPage(entry.page);
      // Klickfläche für Sprung zur Kapitelseite
      doc.link(entry.x - 2, entry.y, entry.w, entry.h, { pageNumber: targetPage });
      // Seitenzahl rechts neben TOC-Eintrag (in dezenter Farbe)
      doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(150, 150, 150);
      doc.text(`S. ${targetPage}`, lm + pw - 10, entry.ySlot, { align: 'right' });
    });
